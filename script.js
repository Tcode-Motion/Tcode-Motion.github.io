...
// 5. 3D Holographic Parser Constellation Grid (Three.js WebGL)
function initThreeJSParser() {
  ...
  // Particles Setup (Abstract Syntax Tree Nodes)
  const particleCount = 180;
  const r = 260; // bounding sphere radius
  ...
  // Create canvas texture for round glowing neon points
  const pTexture = createParticleTexture('#0df28b');
  ...
  // Line Segments for Parser Connections
  ...
  const lineSystem = new THREE.LineSegments(lGeometry, lMaterial);
  ...
  function animate() {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      // Bounce inside sphere radius boundaries
    ...
    // 2. Generate Line Connections
    const linePositions = [];
    const lineColors = [];
    const colorAccent = new THREE.Color('#0df28b'); // Neon Green
    const colorAccent2 = new THREE.Color('#38bdf8'); // Sky Blue
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      for (let j = i + 1; j < particleCount; j++) {
        ...
        if (dist < maxDistance) {
          ...
          // Mixed color based on distance ratio
          const ratio = 1 - (dist / maxDistance);
          const mixedColor = colorAccent.clone().lerp(colorAccent2, ratio * 0.45);
          lineColors.push(mixedColor.r, mixedColor.g, mixedColor.b, mixedColor.r, mixedColor.g, mixedColor.b);
        }
      }
    }

    lGeometry.setAttribute('position', new THREE.Float32BufferAttribute(linePositions, 3));
    lGeometry.setAttribute('color', new THREE.Float32BufferAttribute(lineColors, 3));
    lMaterial.vertexColors = true;
    lGeometry.attributes.position.needsUpdate = true;

    // 3. Camera Parallax Drift
    targetX += (mouseX - targetX) * 0.05;
    targetY += (mouseY - targetY) * 0.05;

    camera.position.x = targetX * 180;
    camera.position.y = -targetY * 180;
    camera.lookAt(scene.position);

    // 4. Auto-rotation drift
  ...
  // Draw smooth round circular radial texture
  function createParticleTexture(colorStr) {
    ...
    grad.addColorStop(0.35, 'rgba(13, 242, 139, 0.7)');
